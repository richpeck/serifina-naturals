...
class SinatraApp < Sinatra::Base
  ...
  get '/' do
    ...
    shopify_session do |shop_name|
      ...
      @charms   = Charm.all
      @stones   = Stone.all.joins(:stone_type).select("stones.id, stones.name, stones.price, nodes.name AS stone_type_name_item").order(stone_type_name_item: :asc).group(stone_type_name_item: :asc).references(:stone_type)
  ...
  get '/select' do
    ...
    @shape  = Shape.find_by(id: params[:shape])
    @charms = @shape.charms.joins(:charm_type).select("charms.id, charms.name, charms.price, nodes.name AS charm_type_name_item").order(charm_type_name_item: :asc).references(:charm_type) if @shape
  ...
  post '/select' do
    ...
    @stones.each_with_index do |stone,index|
      stones << {"name": "Stone #{index + 1}", "value": "#{stone.name.titleize} (#{stone.stone_type_name.titleize}) (+ #{number_to_currency(stone.price)})"}
    end
  ...
  route :post, :put, :delete, '/edit' do # => /edit (charm_type,15)
    ...
    halt 403, "Invalid Params" unless %w(shape shape_price shape_type charm charm_type charm_price stone stone_price).include? @type
