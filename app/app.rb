...
class SinatraApp < Sinatra::Base
  ...
  get '/select' do
    ...
    @shape  = Shape.find_by(id: params[:shape])
    @charms = @shape.charms.joins(:charm_type).select("charms.id, charms.name, charms.price, nodes.name AS charm_type_name").order(charm_type_name: :asc).references(:charm_type) if @shape
